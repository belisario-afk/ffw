/* NOTE: This is your existing file with two targeted fixes:
   1) Mosaic floor no longer depends on floorTex to render. It uses a placeholder until album art arrives.
   2) Aventador wireframe gains a ThinLines fallback tied to the same 'fat-lines don't draw' check.
*/
...
function defaults(initial?: any): LocalCfg {
  return {
    ...
    cameraPresets: { enabled: true, strength: 0.7 },
    fx: {
      beams: false,
      groundRings: false,
      chimneySmoke: false,
      ...
      bloomBreath: false,
      lyricsMarquee: true,
      mosaicFloor: true,   // enable by default so you see it
      wireCar: true
    }
  }
}
...
export default function WireframeHouse3D({ auth, quality, accessibility, settings }: Props) {
  ...
  useEffect(() => {
    ...
    // Mosaic (FIX: decouple from album art; use placeholder until cover arrives)
    ...
    mosaicGroup.visible = !!cfgRef.current.fx.mosaicFloor // FIX: no dependency on floorTex
    ...
    const loadAlbumCoverAndLyrics = async () => {
      try {
        ...
        if (url) {
          ...
          // Keep mosaic visible even if album art fails; update tiles' maps when cover arrives.
          if (tiles.length) {
    ...
    // Reactivity (for palette breathing)
    ...
    const animate = () => {
      ...
      // Mosaic toggle (FIX: independent of album art)
      ...
      // Fallback detector: if shader Lines2 aren’t drawing on this device, swap both mansion and car to thin lines
      frames++
      if (!fallbackArmed && frames > 12) {
        const calls = (renderer.info.render.calls || 0)
        if (calls <= 1) { // likely nothing but the post chain got drawn
          fallbackArmed = true
          fatLines.visible = false
          thinLines.visible = true
          aventador.useThin(true) // FIX: make car visible on devices where Lines2 fails
        }
      }
    }
  ...
  // setResolution (ONLY defined once)
  ...
  const arcXZ = (cx:number, y:number, cz:number, rx:number, rz:number, a0:number, a1:number, steps:number) => {
    ...
    for (let i=0;i<=steps;i++){ const t = THREE.MathUtils.lerp(a0, a1, i/steps); pts.push(V(cx + Math.cos(t)*rx, y, cz + Math.sin(t)*rz)) }
    poly(pts)
  }
  const circleYZ = (cx:number, cy:number, cz:number, r:number, steps:number) => {
    const pts: THREE.Vector3[] = []
    for (let i=0;i<=steps;i++){ const t = (i/steps)*Math.PI*2; pts.push(V(cx, cy + Math.cos(t)*r, cz + Math.sin(t)*r)) }
    poly(pts, true)
  }
  ...
  return { group, setResolution, update, dispose, setColor, useThin }
}

  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [quality.renderScale, quality.bloom, accessibility.epilepsySafe, auth])

  // UI helpers and components unchanged...
  const Joystick = (props: { size?: number; onChange: (nx: number, ny: number) => void; label?: string }) => { /* unchanged */ const size = props.size ?? 110; const radius = size/2; const knobRef = useRef<HTMLDivElement|null>(null); const baseRef = useRef<HTMLDivElement|null>(null); const dragRef = useRef<{active:boolean; cx:number; cy:number}>({ active:false, cx:0, cy:0 }); const setKnob=(nx:number,ny:number)=>{ const k=knobRef.current; if(!k) return; const x=nx*(radius-14), y=ny*(radius-14); k.style.transform=`translate(${x}px, ${y}px)`}; const onPointerDown=(e:React.PointerEvent)=>{ const rect=baseRef.current!.getBoundingClientRect(); dragRef.current.active=true; dragRef.current.cx=rect.left+rect.width/2; dragRef.current.cy=rect.top+rect.height/2; (e.target as HTMLElement).setPointerCapture?.((e as any).pointerId); onPointerMove(e)}; const onPointerMove=(e:React.PointerEvent)=>{ if(!dragRef.current.active) return; const dx=e.clientX-dragRef.current.cx; const dy=e.clientY-dragRef.current.cy; const dist=Math.min(1, Math.hypot(dx,dy)/(radius-14)); const ang=Math.atan2(dy,dx); const nx=Math.cos(ang)*dist; const ny=Math.sin(ang)*dist; setKnob(nx,ny); props.onChange(nx,ny)}; const end=(e?:React.PointerEvent)=>{ dragRef.current.active=false; setKnob(0,0); props.onChange(0,0); if(e) (e.target as HTMLElement).releasePointerCapture?.((e as any).pointerId)}; return (<div style={{ display:'inline-flex', flexDirection:'column', alignItems:'center', gap:6 }}><div ref={baseRef} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={end} onPointerCancel={end} style={{ position:'relative', width:size, height:size, borderRadius:'50%', background:'radial-gradient(ellipse at center, rgba(40,48,60,0.65), rgba(15,18,24,0.85))', border:'1px solid #2b2f3a', boxShadow:'inset 0 2px 14px rgba(0,0,0,0.4)', touchAction:'none', userSelect:'none' }}><div ref={knobRef} style={{ position:'absolute', left:'50%', top:'50%', width:28, height:28, marginLeft:-14, marginTop:-14, borderRadius:'50%', background:'linear-gradient(180deg,#cfe7ff,#9fb7d6)', boxShadow:'0 2px 8px rgba(0,0,0,0.35)', border:'1px solid #5a6b84', transform:'translate(0px,0px)' }}/></div>{props.label && <div style={{ fontSize:10, opacity:0.75 }}>{props.label}</div>}</div>) }

  const Rail = (props: { length?: number; vertical?: boolean; onChange: (n: number) => void; value?: number; label?: string }) => { /* unchanged */ const length=props.length??120; const vertical=!!props.vertical; const railRef=useRef<HTMLDivElement|null>(null); const knobRef=useRef<HTMLDivElement|null>(null); const dragRef=useRef<{active:boolean;min:number;max:number}>({active:false,min:0,max:0}); useEffect(()=>{ const k=knobRef.current; if(!k) return; const v=THREE.MathUtils.clamp(props.value??0,-1,1); if(vertical) k.style.transform=`translate(0px, ${((1 - v) * 0.5 * length) - (length/2)}px)`; else k.style.transform=`translate(${((v + 1) * 0.5 * length) - (length/2)}px, 0px)`},[props.value,vertical,length]); const start=(e:React.PointerEvent)=>{ const rect=railRef.current!.getBoundingClientRect(); dragRef.current.active=true; dragRef.current.min=vertical?rect.top:rect.left; dragRef.current.max=vertical?rect.bottom:rect.right; (e.target as HTMLElement).setPointerCapture?.((e as any).pointerId); move(e)}; const move=(e:React.PointerEvent)=>{ if(!dragRef.current.active) return; const pos=vertical?e.clientY:e.clientX; const t=THREE.MathUtils.clamp((pos-dragRef.current.min)/Math.max(1,(dragRef.current.max-dragRef.current.min)),0,1); const v=vertical?(1-t)*2-1:t*2-1; if(knobRef.current){ if(vertical) knobRef.current.style.transform=`translate(0px, ${((1 - v) * 0.5 * length) - (length/2)}px)`; else knobRef.current.style.transform=`translate(${((v + 1) * 0.5 * length) - (length/2)}px, 0px)`} props.onChange(v)}; const end=(e:React.PointerEvent)=>{ dragRef.current.active=false; (e.target as HTMLElement).releasePointerCapture?.((e as any).pointerId)}; return (<div style={{ display:'inline-flex', flexDirection:'column', alignItems:'center', gap:6 }}><div ref={railRef} onPointerDown={start} onPointerMove={move} onPointerUp={end} onPointerCancel={end} style={{ width:vertical?18:length, height:vertical?length:18, borderRadius:9, background:'linear-gradient(180deg, rgba(28,34,44,0.9), rgba(18,22,28,0.9))', border:'1px solid #2b2f3a', position:'relative', touchAction:'none', userSelect:'none' }}><div ref={knobRef} style={{ position:'absolute', left: vertical ? 3 : '50%', top: vertical ? '50%' : 3, width:12, height:12, marginLeft: vertical ? 0 : -6, marginTop: vertical ? -6 : 0, borderRadius:6, background:'linear-gradient(180deg,#cfe7ff,#9fb7d6)', border:'1px solid #5a6b84', boxShadow:'0 2px 6px rgba(0,0,0,0.35)', transform:'translate(0px,0px)' }}/></div>{props.label && <div style={{ fontSize:10, opacity:0.75 }}>{props.label}</div>}</div>) }
