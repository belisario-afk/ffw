...
export default function OrigamiFold({ quality, accessibility }: Props) {
    ...
    useEffect(() => {
        ...
        // Rotate vector v around axis (unit) by angle
        vec3 rotateVec(vec3 v, vec3 axis, float ang){
          float c = cos(ang), s = sin(ang);
          return v*c + cross(axis, v)*s + axis*dot(axis, v)*(1.0 - c);
        }

        // Rotate point p around a line defined by axis (unit) passing through point q
        ...
        void main(){
          ...
          // Sheet is centered at 0 with size 1.4; hinge lines pass through origin for simplicity.

          // Fold 1: vertical hinge (axis = +Y), fold right half (x > 0)
          if (pos.x > 0.0){
            float ang1 = 3.14159265 * uFold1; // up to 180deg
            vec3 axis1 = vec3(0.0, 1.0, 0.0);
            // hinge runs along x=0, any y; so point on line = (0, pos.y, 0)
            vec3 q1 = vec3(0.0, pos.y, 0.0);
            pos = rotateAroundLine(pos, axis1, q1, ang1);
            nrm = rotateVec(nrm, axis1, ang1);
          }

          // Fold 2: horizontal hinge (axis = +X), fold top half (y > 0)
          if (pos.y > 0.0){
            float ang2 = 3.14159265 * uFold2;
            vec3 axis2 = vec3(1.0, 0.0, 0.0);
            // hinge runs along y=0, any x; point on line = (pos.x, 0, 0)
            vec3 q2 = vec3(pos.x, 0.0, 0.0);
            pos = rotateAroundLine(pos, axis2, q2, ang2);
            nrm = rotateVec(nrm, axis2, ang2);
          }

          // Fold 3: diagonal hinge through center, axis along (1,1,0)/sqrt(2)
          if (pos.x + pos.y > 0.0){
            float ang3 = 3.14159265 * uFold3;
            vec3 axis3 = normalize(vec3(1.0, 1.0, 0.0));
            // point on diagonal line: project pos onto axis3
      ...
      `,
        ...
        void main(){
          ...
          // Vignette
          vec2 q = gl_FragCoord.xy / vec2(${Math.max(1, window.innerWidth)}.0, ${Math.max(1, window.innerHeight)}.0); // will be overridden by post but keep mild
          q = q * 2.0 - 1.0;
          float vig = 1.0 - clamp(dot(q,q)*0.35, 0.0, 1.0);
          col *= mix(1.0, pow(vig, 1.4), clamp(uVignette, 0.0, 1.0));
      ...
      `
        ...
        const animate = () => {
            ...
            setters.setF('uContrastBoost', accessibility.highContrast ? 1.0 : 0.0)
