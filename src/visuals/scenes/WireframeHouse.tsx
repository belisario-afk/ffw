// Wireframe House — upright mesh at origin with a true orbiting camera.
// Consumes ReactiveBus frames (beats/bars/sections) for precise motion.
// Includes party FX: beams, rings, confetti, EQ edges, jitter, windows.
...
export default function WireframeHouse({ auth, quality, accessibility, settings }: Props) {
  ...
  // Subscribe to reactivity bus frames
  useEffect(() => {
  ...
  // Ensure an analyzer is running (feeds bus when <audio> is analyzable; harmless otherwise)
  useEffect(() => {
    let cancelled = false
    async function boot() {
      if (!auth) return
  ...
  function stepPhysics(f: ReactiveFrame | null, dt: number) {
    ...
    // Smooth camera preset lerps toward user base (keeps controls meaningful)
  ...
  function drawScene(g: CanvasRenderingContext2D, canvas: HTMLCanvasElement, f: ReactiveFrame | null) {
    ...
    if (usingFallback) {
      ...
      g.fillRect(12, 12, 170, 22)
      g.fillStyle = 'rgba(255,255,255,0.6)'
      g.font = '12px system-ui, sans-serif'
      g.fillText('Reactivity: fallback/scheduled', 18, 28)
    }

    // Camera (true orbit; house stays still)
