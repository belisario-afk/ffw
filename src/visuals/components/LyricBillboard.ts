...
export class LyricBillboard {
  ...
  private async createLayer(textStr: string): Promise<Layer> {
    ...
    // Underline (grows with progress from left to right)
    const underlineGeom = new THREE.PlaneGeometry(bounds.width, 0.02, 1, 1)
    const underlineMat = new THREE.MeshBasicMaterial({
      color: this.highlightColor.getHex(),
      transparent: true,
      opacity: 0.95,
      depthWrite: false,
      toneMapped: false
    ...
    underline.scale.x = 0 // start at 0% width
  ...
  private applyColors(layer: Layer) {
    ...
    layer.underline.material.color = new THREE.Color(this.highlightColor).getHex()
  ...
  private updateUnderline(layer: Layer, prog: number) {
    ...
    // Plane geometry is centered; reposition so left edge stays at minX and right edge moves with progress
